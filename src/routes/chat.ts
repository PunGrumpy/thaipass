...
export const chatRoutes = new Elysia()
  ...
  .post(
    "/v1/chat/completions",
    {
      body: "ChatRequest",
      detail: {
        description: `Streams by default. Send stream: false for one buffered reply. Multi-turn conversations are flattened into a single role-labelled turn before they reach AI Pass, and token counts in usage are always zero because the upstream reports none. usage.credits reports the account's credit balance and what this reply spent instead; on a stream it is on the final chunk. Omitting model uses ${DEFAULT_MODEL}. Tools are offered to the model through the prompt and its calls are parsed back out of the reply, since AI Pass carries text only; how well that works depends on the model following the format.`,
        responses: {
          "200": jsonOrStream(
            "ChatCompletion",
