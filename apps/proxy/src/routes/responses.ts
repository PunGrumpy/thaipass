...
export const responsesRoutes = new Elysia()
  ...
  .post(
    "/v1/responses",
    {
      ...
      detail: {
        description: `The protocol Codex and the newer OpenAI clients speak. Buffered by default, as the Responses API is. Send stream: true for the event stream, which runs from response.created to response.completed and sends no [DONE] line. The proxy flattens instructions and the whole input list into a single role-labelled turn before it reaches AI Pass, and estimates the token counts in usage from the text because the upstream reports none. usage.credits reports the account's credit balance and what this reply spent instead, and the x-codex-primary-* and x-codex-credits-* headers carry the balance as the turn began, for Codex, which reads its limits there. Omitting model uses ${DEFAULT_MODEL}. The proxy stores no reply, so it refuses previous_response_id and every turn carries the whole conversation. It offers function tools to the model through the prompt and parses its calls back out of the reply as function_call items, since AI Pass carries text only; it drops a hosted tool such as web_search rather than offering the model something it cannot run.`,
        responses: {
          "200": jsonOrStream(
            "Response",
