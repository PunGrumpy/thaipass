...
/** Cookie Store where the browser has it, the document fallback where it does not. */
const writeSidebarCookie = (open: boolean): void => {
  // SAFETY: Cookie Store is not in every lib.dom yet; the guard below is
  // what decides whether it is actually there.
  ...
  // oxlint-disable-next-line unicorn/no-document-cookie -- the fallback for browsers without the Cookie Store API.
...
const SidebarProvider = ({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // Remembers the sidebar state across visits.
...
const SidebarMenuSkeleton = ({
  ...
  // The width is randomised once and never set again, so there is no setter.
  // oxlint-disable-next-line react/hook-use-state -- read-only by design.
