...
const TAP_SPRING = { damping: 30, stiffness: 500, type: "spring" } as const;
const SWAP_SPRING = { bounce: 0, duration: 0.3, type: "spring" } as const;
const CHECK_SPRING = { bounce: 0.35, duration: 0.4, type: "spring" } as const;
...
function hexToHsl(hex: string): [number, number, number] {
  ...
  if (value.length !== 6 || [r, g, b].some(Number.isNaN)) {
    return FALLBACK_HSL;
  }
...
function buildTheme(accent: string, mode: "dark" | "light" = "dark") {
  ...
  const theme: PrismTheme = {
    plain: { backgroundColor: "transparent", color: colors.plain },
    styles: [
      {
        style: { color: tint(ramp(42), s * 0.35), fontStyle: "italic" },
        types: ["comment", "prolog", "doctype", "cdata"],
      },
      { style: { color: tint(ramp(62), s * 0.3) }, types: ["punctuation"] },
      {
      ...
      { style: { color: tint(ramp(88), s * 0.5) }, types: ["function"] },
      {
      ...
      { style: { color: tint(ramp(72), s * 0.6) }, types: ["regex"] },
    ],
  };
